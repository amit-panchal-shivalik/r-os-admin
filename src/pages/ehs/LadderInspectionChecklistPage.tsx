import { useCallback, useEffect, useMemo, useState } from 'react';
import { Controller, useFieldArray, useForm } from 'react-hook-form';
import {
  ActionIcon,
  Alert,
  Badge,
  Box,
  Button,
  Card,
  Group,
  Loader,
  Modal,
  ScrollArea,
  Select,
  SimpleGrid,
  Stack,
  Table,
  Text,
  TextInput,
  Textarea,
} from '@mantine/core';
import { DateInput } from '@mantine/dates';
import { IconAlertCircle, IconEye, IconPencil, IconPlus, IconPrinter, IconRefresh, IconTrash } from '@tabler/icons-react';
import dayjs from 'dayjs';
import EhsPageLayout from '@/components/ehs/EhsPageLayout';
import { usePermissions } from '@/hooks/usePermissions';
import { useSites } from '@/hooks/useSites';
import { useLadderInspections } from '@/hooks/useLadderInspections';
import { LadderInspectionPayload } from '@/apis/ehs';
import { showMessage } from '@/utils/Constant';
import logo from '@/assets/ehs/Logo.jpeg';

const createEntry = () => ({
  ladderId: '',
  material: '',
  location: '',
  portable: '',
  condition: '',
  inspectionDate: new Date(),
  inspectedBy: '',
  remarks: '',
});

type LadderEntryFormValue = ReturnType<typeof createEntry>;

type LadderInspectionFormValues = {
  siteId?: string;
  frequency: string;
  projectInCharge: string;
  notes: string;
  entries: LadderEntryFormValue[];
};

const DEFAULT_VALUES: LadderInspectionFormValues = {
  siteId: undefined,
  frequency: '1st week of every month',
  projectInCharge: '',
  notes: '',
  entries: Array.from({ length: 10 }).map(() => createEntry()),
};

const formatDate = (value?: string | Date | null) => {
  if (!value) return '—';
  const date = value instanceof Date ? value : new Date(value);
  if (Number.isNaN(date.getTime())) return '—';
  return dayjs(date).format('DD/MM/YYYY');
};

const buildPrintHtml = (record: any) => {
  const rows = (record.entries || []).map((entry: any, index: number) => `
      <tr>
        <td style="border:1px solid #000;padding:6px;text-align:center;">${index + 1}</td>
        <td style="border:1px solid #000;padding:6px;">${entry.ladderId ?? ''}</td>
        <td style="border:1px solid #000;padding:6px;">${entry.material ?? ''}</td>
        <td style="border:1px solid #000;padding:6px;">${entry.location ?? ''}</td>
        <td style="border:1px solid #000;padding:6px;text-align:center;">${entry.portable ?? ''}</td>
        <td style="border:1px solid #000;padding:6px;">${entry.condition ?? ''}</td>
        <td style="border:1px solid #000;padding:6px;text-align:center;">${formatDate(entry.inspectionDate)}</td>
        <td style="border:1px solid #000;padding:6px;">${entry.inspectedBy ?? ''}</td>
        <td style="border:1px solid #000;padding:6px;">${entry.remarks ?? ''}</td>
      </tr>
    `);

  const html = `
    <html>
      <head>
        <meta charset="utf-8" />
        <title>Ladder Inspection Checklist</title>
        <style>
          body { font-family: Arial, sans-serif; margin: 0; padding: 24px; background: #fff; color: #000; }
          .sheet { border: 2px solid #000; padding: 20px; }
          .header { display: grid; grid-template-columns: 2fr 3fr 1.2fr; gap: 12px; align-items: stretch; margin-bottom: 16px; }
          .logo { border: 1px solid #000; display: flex; align-items: center; justify-content: center; padding: 12px; background: #fff; }
          .title { border: 1px solid #000; background: #f3f3f3; display: flex; align-items: center; justify-content: center; text-transform: uppercase; font-size: 22px; font-weight: 700; }
          .meta { border: 1px solid #000; background: #f3f3f3; padding: 12px; font-size: 12px; line-height: 1.4; }
          table { width: 100%; border-collapse: collapse; font-size: 12px; }
          th, td { border: 1px solid #000; padding: 6px; }
          th { background: #f3f3f3; text-transform: uppercase; font-size: 11px; }
          .parameters { font-size: 12px; margin-bottom: 12px; }
        </style>
      </head>
      <body>
        <div class="sheet">
          <div class="header">
            <div class="logo"><img src="${logo}" alt="Shivalik" style="max-width:150px;max-height:60px;object-fit:contain;" /></div>
            <div class="title">Ladder Inspection Checklist</div>
            <div class="meta">
              <div>Format No.: EHS-F-21</div>
              <div>Rev. No.: 00</div>
            </div>
          </div>

          <div class="parameters">
            <strong>Ladder checking parameters:</strong>
            <ol>
              <li>Are any parts broken, cracked or splintered?</li>
              <li>Are there any defects in the side rails, rungs or other parts?</li>
              <li>Are there any missing or broken rungs? (loose or moveable by hand)</li>
              <li>Are there any makeshift repairs on the ladder?</li>
              <li>Are there any worn, damaged or missing feet?</li>
              <li>Are there any worn, damaged or unworkable extension ladder locks, pulleys or other fittings?</li>
              <li>Is the rope on extension ladders worn, broken or frayed?</li>
              <li>Has the rope been replaced with an inferior to the ladder manufacturer’s original rope?</li>
              <li>Are the spreader arms on step ladders bent, worn, broken or otherwise rendered partly or totally ineffective?</li>
              <li>Is the ladder twisted, warped or bowed?</li>
            </ol>
          </div>

          <div style="margin-bottom:8px;font-size:12px;">
            <strong>Frequency:</strong> ${record.frequency || '1st week of every month.'}
          </div>

          <table>
            <thead>
              <tr>
                <th style="width:50px;">Sr. No.</th>
                <th>Ladder ID</th>
                <th>Material</th>
                <th>Location</th>
                <th>Portable / Fixed</th>
                <th>Condition (as per above points)</th>
                <th>Inspection date</th>
                <th>Inspected by</th>
                <th>Remarks</th>
              </tr>
            </thead>
            <tbody>
              ${rows.join('')}
            </tbody>
          </table>

          <div style="margin-top:16px;font-size:12px;">
            Project In charge Sign: ${record.projectInCharge || '____________________'}
          </div>
        </div>
      </body>
    </html>
  `;

  return html;
};

const LadderInspectionChecklistPage = () => {
  const { can, loading: permissionsLoading, roles } = usePermissions();
  const { sites, loading: sitesLoading } = useSites();
  const { records, loading, fetchInspections, createInspection, updateInspection } = useLadderInspections({ limit: 50 });

  const [formOpened, setFormOpened] = useState(false);
  const [viewOpened, setViewOpened] = useState(false);
  const [selectedRecord, setSelectedRecord] = useState<any>(null);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);

  const isSuperAdmin = roles.includes('SuperAdmin');
  const canView = isSuperAdmin || can('LadderInspection', 'view') || roles.length === 0;
  const canCreate = isSuperAdmin || can('LadderInspection', 'add') || roles.length === 0;
  const canEdit = isSuperAdmin || can('LadderInspection', 'edit');

  const {
    control,
    handleSubmit,
    register,
    reset,
    setValue,
    watch,
    formState: { errors },
  } = useForm<LadderInspectionFormValues>({ defaultValues: DEFAULT_VALUES });

  const { fields, append, remove, replace } = useFieldArray({ control, name: 'entries' });
  const watchedSiteId = watch('siteId');

  useEffect(() => {
    if (!permissionsLoading && canView) {
      fetchInspections().catch(() => undefined);
    }
  }, [permissionsLoading, canView, fetchInspections]);

  useEffect(() => {
    if (!watchedSiteId) return;
    const site = sites.find((item) => item._id === watchedSiteId);
    if (site && !watch('projectInCharge')) {
      setValue('projectInCharge', site.name ?? '', { shouldDirty: false });
    }
  }, [watchedSiteId, sites, setValue, watch]);

  const siteOptions = useMemo(
    () =>
      sites.map((site) => ({
        value: site._id,
        label: site.name,
        description: site.location ?? '',
      })),
    [sites]
  );

  const openCreateModal = () => {
    if (!canCreate) {
      showMessage('You do not have permission to log ladder inspections', 'error');
      return;
    }
    reset(DEFAULT_VALUES);
    replace(DEFAULT_VALUES.entries);
    setEditingId(null);
    setFormOpened(true);
  };

  const handleEdit = (record: any) => {
    if (!canEdit) {
      showMessage('You do not have permission to edit ladder inspections', 'error');
      return;
    }
    setEditingId(record._id);
    setFormOpened(true);
    reset({
      siteId: record.siteSnapshot?.id ?? undefined,
      frequency: record.frequency ?? '1st week of every month',
      projectInCharge: record.projectInCharge ?? '',
      notes: record.notes ?? '',
      entries: (record.entries || DEFAULT_VALUES.entries).map((entry: any) => ({
        ladderId: entry.ladderId ?? '',
        material: entry.material ?? '',
        location: entry.location ?? '',
        portable: entry.portable ?? '',
        condition: entry.condition ?? '',
        inspectionDate: entry.inspectionDate ? new Date(entry.inspectionDate) : new Date(),
        inspectedBy: entry.inspectedBy ?? '',
        remarks: entry.remarks ?? '',
      })),
    });
    replace(
      (record.entries || DEFAULT_VALUES.entries).map((entry: any) => ({
        ladderId: entry.ladderId ?? '',
        material: entry.material ?? '',
        location: entry.location ?? '',
        portable: entry.portable ?? '',
        condition: entry.condition ?? '',
        inspectionDate: entry.inspectionDate ? new Date(entry.inspectionDate) : new Date(),
        inspectedBy: entry.inspectedBy ?? '',
        remarks: entry.remarks ?? '',
      }))
    );
  };

  const handleView = (record: any) => {
    setSelectedRecord(record);
    setViewOpened(true);
  };

  const handlePrint = useCallback((record?: any) => {
    const target = record || selectedRecord;
    if (!target) {
      showMessage('Select a record to print', 'info');
      return;
    }
    const html = buildPrintHtml(target);
    const win = window.open('', '_blank');
    if (win) {
      win.document.open();
      win.document.write(html);
      win.document.close();
      win.focus();
      win.print();
      win.close();
    }
  }, [selectedRecord]);

  const onSubmit = async (values: LadderInspectionFormValues) => {
    const payload: LadderInspectionPayload = {
      siteId: values.siteId || undefined,
      frequency: values.frequency || undefined,
      projectInCharge: values.projectInCharge || undefined,
      notes: values.notes || undefined,
      entries: values.entries.map((entry) => ({
        ladderId: entry.ladderId,
        material: entry.material || undefined,
        location: entry.location || undefined,
        portable: entry.portable || undefined,
        condition: entry.condition || undefined,
        inspectionDate: entry.inspectionDate ? entry.inspectionDate.toISOString() : undefined,
        inspectedBy: entry.inspectedBy || undefined,
        remarks: entry.remarks || undefined,
      })),
    };

    setSubmitting(true);
    try {
      if (editingId) {
        await updateInspection(editingId, payload);
      } else {
        await createInspection(payload);
      }
      await fetchInspections();
      setFormOpened(false);
      setEditingId(null);
      reset(DEFAULT_VALUES);
      replace(DEFAULT_VALUES.entries);
    } catch (error: any) {
      showMessage(error?.message ?? 'Unable to save ladder inspection', 'error');
    } finally {
      setSubmitting(false);
    }
  };

  return (
    <EhsPageLayout
      title="Ladder Inspection Checklist"
      description="Ensure portable ladders are defect-free, tagged, and used with stabilizers and anti-skid arrangements."
      actions={
        <Group>
          <Button
            variant="light"
            color="gray"
            leftSection={<IconRefresh size={16} />}
            onClick={() => fetchInspections()}
            loading={loading}
          >
            Refresh
          </Button>
          {canCreate ? (
            <Button leftSection={<IconPlus size={16} />} onClick={openCreateModal}>
              Log Ladder Inspection
            </Button>
          ) : null}
        </Group>
      }
    >
      {permissionsLoading ? (
        <Group justify="center" py="xl">
          <Loader size="sm" />
        </Group>
      ) : !canView ? (
        <Alert color="red" variant="light" icon={<IconAlertCircle size={18} />} title="Access restricted">
          You do not have permission to view ladder inspections.
        </Alert>
      ) : (
        <Stack gap="lg">
          <Card withBorder radius="md" padding="lg" shadow="sm">
            <Stack gap="sm">
              <Group justify="space-between" align="center">
                <Text size="sm" c="dimmed">
                  Inspections recorded: {records.length}
                </Text>
              </Group>
              <ScrollArea>
                <Table highlightOnHover striped withColumnBorders>
                  <thead>
                    <tr>
                      <th>Site</th>
                      <th>Project In Charge</th>
                      <th>Entries</th>
                      <th>Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {records.length ? (
                      records.map((record) => (
                        <tr key={record._id}>
                          <td>{record.siteSnapshot?.name || '—'}</td>
                          <td>{record.projectInCharge || '—'}</td>
                          <td>
                            <Badge color="teal" variant="light">
                              {record.entries?.length ?? 0}
                            </Badge>
                          </td>
                          <td>
                            <Group gap="xs">
                              <ActionIcon variant="subtle" color="blue" onClick={() => handleView(record)}>
                                <IconEye size={16} />
                              </ActionIcon>
                              {canEdit ? (
                                <ActionIcon variant="subtle" color="orange" onClick={() => handleEdit(record)}>
                                  <IconPencil size={16} />
                                </ActionIcon>
                              ) : null}
                              <ActionIcon variant="subtle" color="gray" onClick={() => handlePrint(record)}>
                                <IconPrinter size={16} />
                              </ActionIcon>
                            </Group>
                          </td>
                        </tr>
                      ))
                    ) : (
                      <tr>
                        <td colSpan={4}>
                          <Text size="sm" c="dimmed" ta="center">
                            No ladder inspection data available.
                          </Text>
                        </td>
                      </tr>
                    )}
                  </tbody>
                </Table>
              </ScrollArea>
            </Stack>
          </Card>

          {selectedRecord ? (
            <Card withBorder radius="md" padding="lg" shadow="sm">
              <Group justify="space-between" align="center" mb="md">
                <Text fw={600}>Checklist preview</Text>
                <Group gap="sm">
                  <Button variant="light" color="gray" leftSection={<IconPrinter size={16} />} onClick={() => handlePrint(selectedRecord)}>
                    Print
                  </Button>
                  <Button variant="default" onClick={() => setSelectedRecord(null)}>
                    Close
                  </Button>
                </Group>
              </Group>
              <ScrollArea h={320}>
                <Table withColumnBorders striped>
                  <thead>
                    <tr>
                      <th>Sr</th>
                      <th>Ladder ID</th>
                      <th>Material</th>
                      <th>Location</th>
                      <th>Portable / Fixed</th>
                      <th>Condition</th>
                      <th>Inspection date</th>
                      <th>Inspected by</th>
                      <th>Remarks</th>
                    </tr>
                  </thead>
                  <tbody>
                    {selectedRecord.entries?.length ? (
                      selectedRecord.entries.map((entry: any, index: number) => (
                        <tr key={`${entry.ladderId}-${index}`}>
                          <td>{index + 1}</td>
                          <td>{entry.ladderId || '—'}</td>
                          <td>{entry.material || '—'}</td>
                          <td>{entry.location || '—'}</td>
                          <td>{entry.portable || '—'}</td>
                          <td>{entry.condition || '—'}</td>
                          <td>{formatDate(entry.inspectionDate)}</td>
                          <td>{entry.inspectedBy || '—'}</td>
                          <td>{entry.remarks || '—'}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
                        <td colSpan={9}>
                          <Text size="sm" c="dimmed" ta="center">
                            No entries available.
                          </Text>
                        </td>
                      </tr>
                    )}
                  </tbody>
                </Table>
              </ScrollArea>
            </Card>
          ) : null}
        </Stack>
      )}

      <Modal
        opened={formOpened}
        onClose={() => {
          setFormOpened(false);
          setEditingId(null);
        }}
        size="95%"
        radius="md"
        title={editingId ? 'Edit Ladder Inspection' : 'Log Ladder Inspection'}
        overlayProps={{ blur: 3 }}
      >
        <form onSubmit={handleSubmit(onSubmit)}>
          <ScrollArea offsetScrollbars h="70vh">
            <Stack gap="lg" py="xs">
              <Card withBorder radius="md" padding="lg" shadow="xs">
                <SimpleGrid cols={{ base: 1, md: 2, lg: 4 }} spacing="md">
                  <Controller
                    control={control}
                    name="siteId"
                    render={({ field }) => (
                      <Select
                        label="Site"
                        placeholder={sitesLoading ? 'Loading sites...' : 'Select site'}
                        data={siteOptions}
                        value={field.value ?? null}
                        onChange={(value) => field.onChange(value ?? undefined)}
                        searchable
                        clearable
                      />
                    )}
                  />
                  <TextInput label="Frequency" placeholder="1st week of every month" {...register('frequency')} />
                  <TextInput label="Project In Charge" placeholder="Project manager" {...register('projectInCharge')} />
                  <Textarea label="Notes" minRows={2} autosize {...register('notes')} />
                </SimpleGrid>
              </Card>

              <Card withBorder radius="md" padding="lg" shadow="xs">
                <ScrollArea offsetScrollbars>
                  <Table withColumnBorders horizontalSpacing="sm" verticalSpacing="sm">
                    <thead>
                      <tr>
                        <th style={{ width: 50 }}>Sr</th>
                        <th>Ladder ID</th>
                        <th>Material</th>
                        <th>Location</th>
                        <th>Portable / Fixed</th>
                        <th>Condition</th>
                        <th>Inspection date</th>
                        <th>Inspected by</th>
                        <th>Remarks</th>
                        <th style={{ width: 40 }}></th>
                      </tr>
                    </thead>
                    <tbody>
                      {fields.map((field, index) => (
                        <tr key={field.id}>
                          <td>{index + 1}</td>
                          <td>
                            <TextInput
                              placeholder="LAD-01"
                              {...register(`entries.${index}.ladderId` as const, {
                                required: 'Required',
                              })}
                              error={errors.entries?.[index]?.ladderId?.message}
                            />
                          </td>
                          <td>
                            <TextInput placeholder="Material" {...register(`entries.${index}.material` as const)} />
                          </td>
                          <td>
                            <TextInput placeholder="Location" {...register(`entries.${index}.location` as const)} />
                          </td>
                          <td>
                            <TextInput placeholder="Portable/Fixed" {...register(`entries.${index}.portable` as const)} />
                          </td>
                          <td>
                            <TextInput placeholder="Condition" {...register(`entries.${index}.condition` as const)} />
                          </td>
                          <td>
                            <Controller
                              control={control}
                              name={`entries.${index}.inspectionDate` as const}
                              render={({ field: dateField }) => (
                                <DateInput value={dateField.value} onChange={dateField.onChange} valueFormat="DD MMM YYYY" />
                              )}
                            />
                          </td>
                          <td>
                            <TextInput placeholder="Inspector" {...register(`entries.${index}.inspectedBy` as const)} />
                          </td>
                          <td>
                            <TextInput placeholder="Remarks" {...register(`entries.${index}.remarks` as const)} />
                          </td>
                          <td>
                            <ActionIcon variant="subtle" color="red" onClick={() => remove(index)} disabled={fields.length === 1}>
                              <IconTrash size={16} />
                            </ActionIcon>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                  </Table>
                </ScrollArea>
                <Group justify="flex-end" mt="md">
                  <Button variant="subtle" leftSection={<IconPlus size={14} />} onClick={() => append(createEntry())}>
                    Add row
                  </Button>
                </Group>
              </Card>

              <Group justify="flex-end">
                <Button
                  variant="default"
                  onClick={() => {
                    setFormOpened(false);
                    setEditingId(null);
                  }}
                >
                  Cancel
                </Button>
                <Button type="submit" loading={submitting}>
                  {editingId ? 'Update inspection' : 'Save inspection'}
                </Button>
              </Group>
            </Stack>
          </ScrollArea>
        </form>
      </Modal>

      <Modal
        opened={viewOpened && !!selectedRecord}
        onClose={() => {
          setViewOpened(false);
          setSelectedRecord(null);
        }}
        size="90%"
        radius="md"
        title="Ladder Inspection"
        overlayProps={{ blur: 3 }}
      >
        {selectedRecord ? (
          <Stack gap="md">
            <SimpleGrid cols={{ base: 1, md: 2 }} spacing="md">
              <Card withBorder radius="md" padding="md" shadow="xs">
                <Text size="sm" c="dimmed">
                  Site
                </Text>
                <Text fw={600}>{selectedRecord.siteSnapshot?.name || '—'}</Text>
              </Card>
              <Card withBorder radius="md" padding="md" shadow="xs">
                <Text size="sm" c="dimmed">
                  Project in charge
                </Text>
                <Text fw={600}>{selectedRecord.projectInCharge || '—'}</Text>
              </Card>
            </SimpleGrid>
            <ScrollArea h={320}>
              <Table withColumnBorders striped>
                <thead>
                  <tr>
                    <th>Sr</th>
                    <th>Ladder ID</th>
                    <th>Material</th>
                    <th>Location</th>
                    <th>Portable / Fixed</th>
                    <th>Condition</th>
                    <th>Inspection date</th>
                    <th>Inspected by</th>
                    <th>Remarks</th>
                  </tr>
                </thead>
                <tbody>
                  {selectedRecord.entries?.length ? (
                    selectedRecord.entries.map((entry: any, index: number) => (
                      <tr key={`${entry.ladderId}-${index}`}>
                        <td>{index + 1}</td>
                        <td>{entry.ladderId || '—'}</td>
                        <td>{entry.material || '—'}</td>
                        <td>{entry.location || '—'}</td>
                        <td>{entry.portable || '—'}</td>
                        <td>{entry.condition || '—'}</td>
                        <td>{formatDate(entry.inspectionDate)}</td>
                        <td>{entry.inspectedBy || '—'}</td>
                        <td>{entry.remarks || '—'}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan={9}>
                        <Text size="sm" c="dimmed" ta="center">
                          No entries available.
                        </Text>
                      </td>
                    </tr>
                  )}
                </tbody>
              </Table>
            </ScrollArea>
          </Stack>
        ) : null}
      </Modal>
    </EhsPageLayout>
  );
};

export default LadderInspectionChecklistPage;
