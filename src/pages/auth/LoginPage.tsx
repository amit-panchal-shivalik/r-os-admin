...
export const LoginPage = () => {
    ...
    const { status, error, user: authUser }: any = useSelector((state: any) => state.auth);
    ...
    useEffect(() => {
        if (status === 'complete' && authUser) {
            ...
            const userData = authUser.result || authUser;
            const userInfo = {
                id: userData.id || '',
                name: userData.name || `${userData.firstName || ''} ${userData.lastName || ''}`.trim(),
                email: userData.email || '',
                phone: userData.phone || '',
                role: userData.userRoles?.join(',') || userData.role || '',
            ...
            setTimeout(() => {
                const roles: string[] = userData.userRoles || [];
    ...
    return (
        ...
        >
            <div className="w-full max-w-sm rounded-2xl shadow-lg border border-gray-100 text-center p-6">
                ...
                <h1 className="my-4 text-2xl font-bold text-[#2E2E2E] flex-shrink-0">R-OS</h1>
                ...
                >
                    {({ isSubmitting, values, errors, touched }) => (
                        <Form className="mt-6">
                            ...
                            {/* API Error Message - Fixed height container */}
                            ...
                            <Button
                                type="submit"
                                disabled={status === 'loading' || isSubmitting || !values.email || !values.password}
