...
import { normalizeUserPayload } from "@/utils/user";
...
export const OtpPage = () => {
  ...
  const { user: otpResponse, error, status }: any = useSelector(
    (state: any) => state.otp
  );
  ...
  useEffect(() => {
    if (status === "complete" && otpResponse?.access_token) {
      const normalizedUser = normalizeUserPayload({
        ...otpResponse.user,
        phone: phoneNumber,
      });
      login(normalizedUser, otpResponse.access_token);
      removeFromLocalStorage("user_mobile");
  ...
  }, [status, navigate, login, otpResponse, phoneNumber]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!phoneNumber) {
      showMessage("Missing mobile number. Please login again.", "error");
      return;
    }
