...
import { createUserApi } from "@/apis/auth";
...
export const RegisterPage = () => {
  ...
  const isValidName = useMemo(
    () => namePattern.test(fullName.trim()),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitting(true);
    setTouched({ fullName: true, phone: true, userType: true });
    ...
    try {
      ...
      setPhoneNumber("");
      setUserType("");
      setTouched({ fullName: false, phone: false, userType: false });
      ...
      <div className="w-full max-w-sm rounded-2xl shadow-lg border border-gray-100 text-center p-6">
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <Button
            type="submit"
            disabled={
              submitting || !isValidName || !isValidPhone || !isValidUserType
            }
