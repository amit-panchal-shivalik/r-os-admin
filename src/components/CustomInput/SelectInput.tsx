...
const SelectInput: React.FC<SelectInputProps> = ({
  ...
  const hookFormError = errors?.[id]?.message as string | undefined;
  const mergedError = errorMessage ?? hookFormError;
  const fieldName =
    id && id.replace(/([A-Z])/g, " $1").replace(/^./, (s) => s.toUpperCase());
    ...
    <div className={`input-wrapper material-input-wrapper ${mergedError ? "has-error" : ""}`}>
      <style>
        {`
                    ...
                    .material-input-wrapper.has-error select {
                    ...
                    .material-input-wrapper.has-error select:focus {
