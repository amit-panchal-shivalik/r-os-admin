...
export default function KpiCard({ kpi, delay = 0 }: { kpi: Kpi; delay?: number }) {
  ...
  return (
    <Card withBorder shadow="sm" radius="md" p="xl" style={{ opacity: 0, animation: `fadeIn .6s ${delay}ms forwards` }}>
      <Stack gap="md">
        <Group justify="space-between" align="flex-start">
          <Text size="sm" c="dimmed">{kpi.title}</Text>
          {kpi.targetLabel || kpi.target ? (
            <Badge color={color} variant="light">{kpi.targetLabel ?? `Target ${kpi.target}%`}</Badge>
          ) : null}
        </Group>
        ...
        <Group gap="md" align="center">
          <RingProgress
            ...
            label={<Text fw={700}>{value}%</Text>}
          />
          <Stack gap={6} style={{ flex: 1 }}>
            <Text size="sm" c="dimmed">{kpi.subtitle ?? '—'}</Text>
            <Progress radius="xl" size="sm" value={value} color={color} />
            <Text size="xs" c="dimmed">Progress towards site target</Text>
          </Stack>
        </Group>
      </Stack>
    </Card>
